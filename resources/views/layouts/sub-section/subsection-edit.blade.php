                ...
                <div class="card-body">
                    <form action="{{ route('subsection.update',$subsection->id) }}" method="POST">
                            ...
                            <select class="form-control" type="text" name="section_id">
                                @foreach ($section as $select)
                                    <option value="{{ $select->id }}"
                                        {{ $select->id == $subsection->section_id ? 'selected' : '' }}>{{ $select->area }} |
                                        {{ $select->problem }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group mb-3">
                            ...
                            <input class="form-control" type="text" name="title" value="{{ $subsection->title }}">
                        </div>

                        <div class="form-group mb-3">
                            <label for="my-input">Order</label>
                            <input class="form-control" type="number" name="order" value="{{ $subsection->order }}">
                        </div>
