...
class PaintingController extends Controller
{
    public function painting(Request $request)
    {
        ...
        $auditorsPainting = User::whereHas('answers', function ($query) use ($today) {
    ...
    $totalQuestion = DB::table('fix_answers')
        ...
        ->join('subsections', 'questions.subsection_id', '=', 'subsections.id')
        ->join('sections', 'subsections.section_id', '=', 'sections.id')
        ->whereIn('questions.subsection_id', [19, 20, 21, 22])
        ->whereDate('fix_answers.created_at', $today)
    ...
    foreach ($mark as $markItem) {
        ...
        return view('layouts.painting.dashboardpainting', compact('sectionId', 'mark', 'score', 'auditorsPainting', 'subsectionId', 'questionId', 'answerId', 'totalQuestion', 'judul', 'monthlyPainting', 'today'));
    }
    ...
    public function detailPaintingToday($id)
    {
        ...
        }, 'answers.questions.subsection.sections.part'])->find($id);
        $mark = DB::table('fix_answers')
        ->join('questions', 'fix_answers.question_id', '=', 'questions.id')
        ->join('subsections', 'questions.subsection_id', '=', 'subsections.id')
        ->join('sections', 'subsections.section_id', '=', 'sections.id')
    ...
    foreach ($mark as $markItem) {
        ...
        return view('layouts.painting.detailpainting', compact('judul', 'data', 'totalQuestion', 'auditor','score', 'mark'));
    }

    public function paintingHistory(Request $request)
    {
        $judul = "AUDIT PAINTING HISTORY";
        $today = Carbon::now()->today();
        ...
        // dd($score);
        return view('layouts.painting.paintinghistory', compact('judul', 'data', 'request', 'score', 'totalQuestion', 'mark'));
    }
    ...
    public function detailPaintingHistory($date, $id_user)
    {
        ...
        return view('layouts.painting.detailPaintingHistory', compact(['judul', 'data', 'auditor', 'totalQuestion', 'mark', 'score']));
    }
    ...
    public function auditorsAudit($auditorIds)
    {
        ...
        switch ($auditors->role) {
            ...
            case 4;
                ...
                $queryAuditedAreaBy = Section::where('area', 'Painting')
                ...
                if ($auditors->answers_count == 0) {
                    ...
                    $queryAuditedAreaBy = Section::where('area', 'Painting')->whereHas('subsections.questions.answers', function ($query) use ($today) {
                        $query->whereDate('created_at', $today);
                    ...
                    if (count($queryAuditedAreaBy) > 0) {
                        $auditors = [
                            ...
                            'status' => 4,
                            'auditors' => $auditors,
                            'keterangan' => 'This Painting audit has been done by another Auditor'
                        ];
                    } else {
                        $auditors = [
                            'status' => 1,
            ...
            case 5;
                ...
                if ($auditors->answers_count == 0) {
                    ...
                    $queryAuditedAreaBy = Section::where('area', 'Painting')->whereHas('subsections.questions.answers', function ($query) use ($startOfWeek, $endOfWeek) {
                        $query->whereBetween('created_at', [$startOfWeek, $endOfWeek]);
                    })->whereHas('subsections.questions.answers.auditors', function ($query) {
                        $query->where('role', 5);
                    })
    ...
    public function addEmptyDatePainting()
    {
        $judul = "CHECKSHEET EMPTY DAY PAINTING";
        ...
        return view('layouts.painting.addPaintingAuditPreviousDay', $data, compact('judul', 'auditors', 'sections'));
    }



    public function answerEmptyPainting(Request $request)
    {
        ...
        return redirect()->to('painting-history');
    }
}
